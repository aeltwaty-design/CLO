...
| Review comments (pin layer) | overlay on every screen | prototype tool, not product UI: 💬 chip toggles tap-to-pin commenting (write/edit/delete, author asked once); pins keyed per screen variant, anchored to scroll content; shared via `/api/comments` (`server/comments.mjs` + vite middleware; `npm run serve` hosts dist + API), localStorage + تصدير/استيراد fallback; invisible to webdriver/`?diff=` so the gate never sees it |
