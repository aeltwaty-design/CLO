...
/** Prototype version switch — Phase 1 = the frozen approved experience,
    Phase 2 = the working copy for upcoming iterations,
    ...
    before anything is promoted into Phase 2 — and the **default tab** while
    that review runs (user direction, 2026-08-19). Resolved once per load:
    `?phase=` wins, then the browser tab's sessionStorage (so a mid-flow
    refresh stays in the chosen phase), else 3. The shell's Phase tabs
...
function resolvePhase(): Phase {
  ...
  const phase: Phase = stored === '1' ? 1 : stored === '2' ? 2 : 3;
